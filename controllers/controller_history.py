...
class HistoryController(BaseController):
    ...
    def _load_traffic_statistics(self, video_id: int):
        """Load traffic statistics for video"""
        try:
            ...
            stats = self.traffic_repo.calculate_statistics(video_id)
            ...
            if self._view:
                ...
                <table>
                <tr><td><b>Tổng phương tiện:</b></td><td>{stats['total_vehicles']}</td></tr>
                <tr><td><b>Ô tô:</b></td><td>{stats['vehicle_breakdown']['car']} ({stats.get('car_percentage', 0):.1f}%)</td></tr>
                <tr><td><b>Xe máy:</b></td><td>{stats['vehicle_breakdown']['motorbike']} ({stats.get('motorbike_percentage', 0):.1f}%)</td></tr>
                <tr><td><b>Xe tải:</b></td><td>{stats['vehicle_breakdown']['truck']} ({stats.get('truck_percentage', 0):.1f}%)</td></tr>
                <tr><td><b>Xe buýt:</b></td><td>{stats['vehicle_breakdown']['bus']} ({stats.get('bus_percentage', 0):.1f}%)</td></tr>
                </table>
