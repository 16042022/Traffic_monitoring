...
class TestTrafficDataRepository(BaseTestCase):
    ...
    def test_timestamps(self):
        """Test created_at and updated_at timestamps behavior"""
        ...
        initial_created = traffic_data.created_at
        initial_updated = traffic_data.updated_at
        
        # For new records, created_at and updated_at should be very close (within 1 second)
        time_diff = abs((initial_updated - initial_created).total_seconds())
        self.assertLess(time_diff, 1.0, "created_at and updated_at should be close for new records")
        ...
        # 1. updated_at should be LATER than the initial updated_at
        self.assertGreater(updated.updated_at, initial_updated, 
                          "updated_at should increase after update")
        
        # 2. updated_at should be LATER than created_at
        self.assertGreater(updated.updated_at, updated.created_at,
                          "updated_at should always be >= created_at")
        
        # 3. The time between update and original creation should make sense
        update_delay = (updated.updated_at - initial_created).total_seconds()
        self.assertGreater(update_delay, 0.2, 
                          "Update should happen after the sleep delay")
        
        # Optional: Test multiple updates
        time.sleep(1)
        updated2 = self.repo.update(
            traffic_data.id,
            total_vehicles=150
        )
        
        # Each update should increase updated_at
        self.assertGreater(updated2.updated_at, updated.updated_at,
                          "updated_at should increase with each update")
