...
class TestTrafficDataRepository(BaseTestCase):
    ...
    def test_timestamps(self):
        """Test created_at and updated_at timestamps are properly set"""
        # Create record
        traffic_data = self.repo.create(
            video_id=self.video.id,
            total_vehicles=50
        ...
        # Basic checks - timestamps should exist
        self.assertIsNotNone(traffic_data.created_at, "created_at should be set")
        self.assertIsNotNone(traffic_data.updated_at, "updated_at should be set")
        
        # Both should be datetime objects
        from datetime import datetime
        self.assertIsInstance(traffic_data.created_at, datetime)
        self.assertIsInstance(traffic_data.updated_at, datetime)
        ...
        # After update, timestamps should still exist
        self.assertIsNotNone(updated.created_at)
        self.assertIsNotNone(updated.updated_at)
        
        # That's it! For a demo app, we don't need microsecond precision testing
